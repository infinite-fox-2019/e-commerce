    ...
    <div class="container form-container">
      <div class="notification has-background-white">
        <form @submit.prevent="updateProduct">
          ...
          <div class="control">
            <button type="submit" class="button is-danger" style="width: 200px; float: right; margin-top: 40px;">Update Product</button>
          </div>
        </form>
      </div>
    </div>
...
export default {
  ...
  methods: {
    updateProduct () {
      ...
      if (this.file) {
        this.$store.dispatch('updateProductWithImage', payload)
      } else {
        this.$store.dispatch('updateProduct', payload)
      }
    },
    setProduct () {
      let product = this.$store.state.products.find(x => x._id === this.$route.params.id)
      this.name = product.name
      this.description = product.description
      this.price = product.price
      this.stock = product.stock
      this.selectedSeries = product.series
    }
  },
  created () {
    this.setProduct()
